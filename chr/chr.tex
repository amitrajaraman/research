...
\begin{frame}{Some setup for the proof}
	...
	\begin{itemize}
		...
		\item Finally, define $\lambda_{\mathbb{I}} = \binom{\tau}{\mathbb{I}} n^{-\tau}$, which is the probability of getting the multi-index $\tau$ on uniformly drawing an element of $[n]$ $\tau$ times. \pause
	\end{itemize}
\end{frame}
...
\begin{frame}{A proof of (1)}
	...
	Consider the event $E$ that every $x_i$ is in $K$. Observe that the probability that $\mathcal{G}_v^{(\tau)} = \mathcal{H}_v^{(\tau)}$ is at least the probability that $E$ occurs. \pause Therefore,
...
\begin{frame}{The isoperimetric inequality (2)}
	\begin{align*}
		\vol(\partial_{K'}(\mathcal{C}_2')) &\ge \frac{\log 2}{R_0} \min\{ \vol(\mathcal{C}_2') , \vol(K' \setminus \mathcal{C}_2') \} \\
			...
			\uncover<6->{&= \frac{\log 2}{8R_0} \left( \vol(S_1) - \epsilon \right).}
	\end{align*}
...
\begin{frame}{Putting the two pieces together}
	...
	\begin{align*}
		\vol(S_3) &\ge \frac{\delta}{96n^2 \log n} \vol(\partial_{K'}(\mathcal{C}_2')) \\
			\uncover<2->{&\ge \frac{\delta}{96n^2 \log n} \cdot \frac{\log 2}{8R_0} \left( \vol(S_1) - \epsilon \right)} \\
