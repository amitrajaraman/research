...
\begin{frame}{The problem}
	...
	\begin{defn}
		...
		\begin{itemize}
			...
			\item we have $R>r>0$ such that $r B_2^n \subseteq K \subseteq R B_2^n$. \pause
		\end{itemize}
	\end{defn}
...
\begin{frame}{The approach in broad strokes}
	...
	The first such random walk that gets close to the stationary distribution in polynomial time was proposed in \cite{dyer-frieze-kannan} by Dyer, Frieze, and Kannan. \pause This random walk was on a grid superimposed on the convex body. \pause
	\begin{center}
...
\begin{frame}{Hit-and-run}
	...
	\begin{defn}[Hit-and-run]
		...
		\[ K \cap (x_t + y\R). \]
	\end{defn}
	\pause Since the scheme is ergodic and reversible, its stationary distribution is the uniform distribution over $K$. % ***** CHECK YOURSELF!
\end{frame}

\begin{frame}{Coordinate hit-and-run}
	In the simpler \emph{coordinate hit-and-run} walk (which is the subject of this presentation), we instead draw $y$ u.a.r. from $\{e_1,\ldots,e_n\}$, the standard basis of $\Rn$. \pause \\
	In other words,
...
\begin{frame}{Conductance}
	The primary tool we use to determine how fast the Markov chains mix is \emph{conductance}. \pause
	...
	\begin{defn}[Conductance]
		...
		for measurable $A,B\subseteq S$. We also denote $\Phi_{P,Q}(A) = \Phi_{P,Q}(A,S\setminus A)$. \pause \\
...
\begin{frame}{Some setup for the proof}
	...
	Let $\mathcal{M}_{n,\tau}$ be the set of all $\mathbb{I} = (i_1,\ldots,i_n) \in \N^n$ with $\sum i_j = \tau$. \pause For each such $\mathbb{I}$, define $\mathcal{G}_{v,\mathbb{I}}$ as the Gaussian distribution centered at $v$ with diagonal covariance matrix $\Sigma_{\mathbb{I}}$ defined by $(\Sigma_\mathbb{I})_{jj} = \mathbb{I}_j \sigma^2$. \pause Finally, define $\lambda_{\mathbb{I}} = \binom{\tau}{\mathbb{I}} n^{-\tau}$, which is the probability of getting the multi-index $\tau$ on uniformly drawing an element of $[n]$ $\tau$ times. \pause \\
	Observe that if we drop the $v \in K$ condition in the definition of $\mathcal{G}$, we just get the random walk $\mathcal{H}$ defined by
	\[ \mathcal{H}_{v}^{(\tau)} = \sum_{\mathbb{I} \in \mathcal{M}_{n,\tau}} \lambda_{\mathbb{I}} \mathcal{G}_{v,\mathbb{I}}. \]
	\pause Also define the random walk $\mathcal{H}'$ by
...
\begin{frame}{A proof of (1)}
	\[ d_{\text{TV}}(\mathcal{G}_{v}^{(\tau)} , \mathcal{H}_v^{(\tau)}) \text{ is small.} \]
	\pause Let $x_0 = v$. For $t \in [\tau]$, pick $i \in [n]$ uniformly randomly, $\kappa \sim \mathcal{N}(0,\sigma^2)$, and let $x_{t} = x_{t-1} + \kappa e_i$. \pause This just represents a draw from $\mathcal{H}_v^{(\tau)}$. \pause Consider the event $E$ that every $x_i$ is in $K$. Observe that the probability that $\mathcal{G}_v^{(\tau)} = \mathcal{H}_v^{(\tau)}$ is at least the probability that $E$ occurs. \pause Therefore,
	\[ d_\text{TV}(\mathcal{G}_v^{(\tau)}, \mathcal{H}_v^{(\tau)}) \le 1 - \Pr[E]. \]
	\pause Now, set $\tau = 20 n \log n$ and suppose that $\inf_{z \in \partial K} \norm{v - z}_\infty > 100 \sigma \log n$. \pause An application of the Chernoff bound (to show that no coordinate is chosen many times) together with a Gaussian tail bound (to show that none of the coordinate changes are too large) shows that with high probability, the point remains in $K$.
\end{frame}
...
\begin{frame}{Completing the proof}
	Finally, since the Radon-Nikodym derivative of $\mathcal{C}(x,\cdot)$ with respect to $\mathcal{G}(x,\cdot)$ is bounded below by $\sqrt{2\pi}\sigma/2R$ at any point, \pause
	\begin{align*}
		\Phi_{\mathcal{C},\pi_K}(S,K\setminus S) &\ge \frac{\sqrt{2\pi}\sigma}{2R\tau} \Phi_{\mathcal{G}',\pi_K}(S,K\setminus S) \\
			\uncover<3->{&= \frac{\sqrt{2\pi}\sigma}{2R\tau} \cdot \frac{\nu\delta}{4(D-\delta)} (\min\{\pi_K(S),\pi_K(K\setminus S)\} - \epsilon) }
	\end{align*}
...
\begin{frame}{The main result}
	\begin{thm}
		\label{mainthm-2}
		Let $K$ be a convex body satisfying $B_2^n \subseteq K$ and let $R_0^2 = \expec_{x \sim \pi_K}\norm{x - b_K}_2^2$. Let $S_1, S_2 \subseteq K$ be axis-disjoint. Then, there exists a universal constant $c$ such that
...
\begin{frame}{The lattice}
	...
	Define $\mathcal{C}$ to be the set of hypercubes in the lattice that intersect $S_1$, $\mathcal{C}_1$ as the set of cubes in $\mathcal{C}$ where $S_1$ takes up at least $2/3$ the volume of the cube, and $\mathcal{C}_2$ as $\mathcal{C} \setminus \mathcal{C}_1$.
\end{frame}
...
\begin{frame}{The case where $\mathcal{C}_1$ has many cubes}
	...
	\begin{align*}
		\pi_K(S_3) &\ge \frac{1}{16n\log n} \sum_{\substack{c \in \mathcal{C}_1 \\ c \cap K' \ne \emptyset}} \pi_K(c \cap S_1) \\
			\uncover<3->{&= \frac{1}{16n\log n} \left(\pi_K(\mathcal{C}_1 \cap S_1) - \frac{\epsilon}{2}\right)} \\
...
\begin{frame}{The isoperimetric inequality (1)}
	First, let us show the second part in the overview described on the previous slide. \pause In \cite{KLS}, Kannan, Lov\'{a}sz, and Simonovits show that for any convex body $K$ and $S \subseteq K$ with $\pi_K(S) \le 1/2$,
	\begin{equation}
		% \label{eqn-kls}
...
\begin{frame}{The isoperimetric inequality (2)}
	\begin{align*}
		\vol(\partial_{K'}(\mathcal{C}_2')) &\ge \frac{\log 2}{R_0} \min\{ \vol(\mathcal{C}_2') , \vol(K' \setminus \mathcal{C}_2') \} \\
			...
			\uncover<3->{&\ge \frac{\log 2}{R_0} \min\left\{ \vol(\mathcal{C}_2 \cap K') , \frac{1}{3} \vol(\mathcal{C}_2 \cap K') \right\}} \\
			\uncover<4->{&= \frac{\log 2}{3R_0} \vol(\mathcal{C}_2 \cap K')} \\
			% &\ge \frac{\log 2}{3R_0} \left(\vol(\mathcal{C}_2 \cap S_1) - \frac{\epsilon}{2} \vol(K)\right) \\
			\uncover<5->{&\ge \frac{\log 2}{3R_0} \left(\frac{1}{2}\vol(S_1) - \frac{\epsilon}{2} \vol(K)\right)} \\
			\uncover<6->{&= \frac{\log 2}{6R_0} \left( \vol(S_1) - \epsilon \right).}
	\end{align*}
...
\begin{frame}{Comparing $\vol(S_3)$ and $\vol(\partial_{K'}(\mathcal{C}_2'))$ (1)}
	...
	\pause If $c_1$ intersects $S_1$ at all, then we have two cases:
	\begin{enumerate}
...
\begin{frame}{Putting the two pieces together}
	...
	\begin{align*}
		\vol(S_3) &\ge \frac{\delta}{96n^2 \log n} \vol(\partial_{K'}(\mathcal{C}_2')) \\
			\uncover<2->{&\ge \frac{\delta}{96n^2 \log n} \cdot \frac{\log 2}{6R_0} \left( \vol(S_1) - \epsilon \right)} \\
			\uncover<3->{&= \frac{c\delta}{R_0 n^2 \log n} \left( \vol(S_1) - \epsilon \right)} \\
			\uncover<4->{&= \frac{c'\epsilon}{R_0 n^{3.5} \log n} \left( \vol(S_1) - \epsilon \right),}
	\end{align*}
	\pause\pause\pause completing the proof.
\end{frame}

\begin{frame}{Room for improvement}
	% It is believed that we can do better in Lemma \ref{cube-isoperimetry}, possibly improving the $1/(n\log n)$ factor on the right to a constant.
