...
\subsection{Rigidity Theorems on Partial Linear Spaces}
	...
	\begin{proof}
		...
		Let $\mathcal{S}$ be the set of all used colours (all the values in $\F_p$ that are equal to some $w_P$). We may also assume wlog that $0 \not\in S$ (no such point can be on a line of cardinality $>3$, and any such line of cardinality $2$ must comprise of only colour $0$ points, so we may simple exclude these points). Then, letting $S_P$ be the set of all points that are on a line passing through $P$, we can use the fact that there is at most one line passing through a pair of distinct points to conclude that
		...
		which contradicts \Cref{eqn: xP yP zP joint bound}. Therefore, there exists a size $4$ line $\ell$ through $Q$ contained in $T$. Further, since no proper subset of a line is also a line, $\ell \subseteq \{Q\} \cup X_{-t} \cup X_\alpha$.\\
